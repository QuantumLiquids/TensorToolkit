...
struct TestIndex : public testing::Test {
  ...
  IndexT idx_2sct_out = IndexT(qnscts2, TenIndexDirType::OUT);
  IndexT idx_2sct_in = IndexT(qnscts2, TenIndexDirType::IN);
};


TEST_F(TestIndex, IndexDirection) {
  EXPECT_EQ(idx_default.GetDir(), TenIndexDirType::NDIR);
  EXPECT_EQ(idx_1sct_in.GetDir(), TenIndexDirType::IN);
  EXPECT_EQ(idx_2sct_out.GetDir(), TenIndexDirType::OUT);
}
