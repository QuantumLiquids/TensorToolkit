...
void RunTestQLTensorDotMultiCase(
  ...
  for (auto &coors : GenAllCoors(t.GetShape())) {
      ...
      const float abs_val = std::max(qlten::abs(expected), qlten::abs(actual1));
...
TEST_F(TestQLTensor, TestDotMultiplication) {
  ...
  const auto abs_val = std::max(qlten::abs(expected_scalar), qlten::abs(actual_scalar));
...
TEST_F(TestQLTensor, ElementWiseSquaredNorm) {
  // Test real scalar tensor: should be identical to ElementWiseSquare
  dten_scalar.Random(U1QN());
  auto original_val = dten_scalar.GetElem({});
  dten_scalar.ElementWiseSquaredNorm();
  EXPECT_NEAR(dten_scalar.GetElem({}), original_val * original_val, 1e-5);
  ...
  dten_1d_test.ElementWiseSquaredNorm();
  for (size_t i = 0; i < dten_1d_test.GetShape()[0]; ++i) {
    EXPECT_NEAR(dten_1d_test.GetElem({i}), original_vals[i] * original_vals[i], 1e-5);
  }
  ...
  dten_2d_test.ElementWiseSquaredNorm();
  for (size_t i = 0; i < dten_2d_test.GetShape()[0]; ++i) {
    for (size_t j = 0; j < dten_2d_test.GetShape()[1]; ++j) {
      EXPECT_NEAR(dten_2d_test.GetElem({i, j}),
                  original_vals_2d[i][j] * original_vals_2d[i][j], 1e-5);
  ...
  // Test complex scalar tensor: should compute |z|^2 = a^2 + b^2
  zten_scalar.Random(U1QN());
  auto original_zval = zten_scalar.GetElem({});
  zten_scalar.ElementWiseSquaredNorm();
  float expected_norm2 = original_zval.real() * original_zval.real()
                       + original_zval.imag() * original_zval.imag();
  EXPECT_NEAR(zten_scalar.GetElem({}).real(), expected_norm2, 1e-5);
  EXPECT_NEAR(zten_scalar.GetElem({}).imag(), 0.0, 1e-5);

  // Test complex 1D tensor
  ZQLTensor zten_1d_test(zten_1d_s);
  zten_1d_test.Random(qn0);
  std::vector<QLTEN_ComplexFloat> original_zvals;
  for (size_t i = 0; i < zten_1d_test.GetShape()[0]; ++i) {
    original_zvals.push_back(zten_1d_test.GetElem({i}));
  }
  zten_1d_test.ElementWiseSquaredNorm();
  for (size_t i = 0; i < zten_1d_test.GetShape()[0]; ++i) {
    float expected = original_zvals[i].real() * original_zvals[i].real()
                   + original_zvals[i].imag() * original_zvals[i].imag();
    EXPECT_NEAR(zten_1d_test.GetElem({i}).real(), expected, 1e-5);
    EXPECT_NEAR(zten_1d_test.GetElem({i}).imag(), 0.0, 1e-5);
  }

  // Test free function
  DQLTensor dten_copy_test(dten_1d_s);
  dten_copy_test.Random(qn0);
  auto result = ElementWiseSquaredNorm(dten_copy_test);
  EXPECT_NE(result.GetActualDataSize(), 0);
  EXPECT_EQ(result.Rank(), dten_copy_test.Rank());
  EXPECT_EQ(result.GetShape(), dten_copy_test.GetShape());
}
...
TEST_F(TestQLTensor, AllFinite) {
  ...
  dten_scalar() = std::numeric_limits<QLTEN_Float>::infinity();
  EXPECT_FALSE(dten_scalar.AllFinite());

  DQLTensor dten_1d_test(dten_1d_s);
  dten_1d_test.Fill(qn0, 1.0f);
  EXPECT_TRUE(dten_1d_test.AllFinite());
  dten_1d_test(3) = std::numeric_limits<QLTEN_Float>::quiet_NaN();
  EXPECT_FALSE(dten_1d_test.AllFinite());

  zten_scalar() = QLTEN_ComplexFloat(1.0f, 2.0f);
  EXPECT_TRUE(zten_scalar.AllFinite());
  zten_scalar() = QLTEN_ComplexFloat(
      std::numeric_limits<QLTEN_Float>::infinity(),
      0.0f);
  EXPECT_FALSE(zten_scalar.AllFinite());
  zten_scalar() = QLTEN_ComplexFloat(
      0.0f,
      std::numeric_limits<QLTEN_Float>::quiet_NaN());
  EXPECT_FALSE(zten_scalar.AllFinite());
}
