...
*
* Description: QuantumLiquids/tensor project. Unittests for index combination.
*/

#include "gtest/gtest.h"
#include "qlten/qltensor_all.h"
#include "qlten/tensor_manipulation/index_combine.h"
...
void TestCombinerIntraStruct(const QLTensor<CombinerElemT, QNT> &combiner) {
  auto combiner_dag = Dag(combiner);
  QLTensor<CombinerElemT, QNT> res;
...
TEST(TestIndexCombine, TestCase) {
  auto combiner = IndexCombine<QLTEN_Double, U1QN>(idx_inp1, idx_inp1, OUT);
  ...
  combiner = IndexCombine<QLTEN_Double, U1QN>(idx_inp1, idx_inp1, IN);
  EXPECT_EQ(Div(combiner), qn0);
  EXPECT_EQ(combiner.GetIndexes()[0], idx_inp1);
  EXPECT_EQ(combiner.GetIndexes()[1], idx_inp1);
  TestCombinerIntraStruct(combiner);

  combiner = IndexCombine<QLTEN_Double, U1QN>(idx_in2, idx_in2);
  EXPECT_EQ(combiner.GetIndexes()[0], idx_in2);
  EXPECT_EQ(combiner.GetIndexes()[1], idx_in2);
  TestCombinerIntraStruct(combiner);

  combiner = IndexCombine<QLTEN_Double, U1QN>(idx_in2, idx_in2, IN);
  EXPECT_EQ(combiner.GetIndexes()[0], idx_in2);
  EXPECT_EQ(combiner.GetIndexes()[1], idx_in2);
  TestCombinerIntraStruct(combiner);
}
