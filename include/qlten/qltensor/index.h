...
*
* Description: QuantumLiquids/tensor project. Linear space constructed by a series of
...
@brief Linear space constructed by a series of QNSector, and it can define a
       direction.
*/
#ifndef QLTEN_QLTENSOR_INDEX_H
#define QLTEN_QLTENSOR_INDEX_H
...
class Index : public Hashable, public Streamable, public Showable, public Fermionicable<QNT> {
  ...
  */
  Index(const QNSectorVec<QNT> &qnscts, const TenIndexDirType dir) :
  ...
  */
  Index(void) : Index({}, TenIndexDirType::NDIR) {}
  ...
  */
  TenIndexDirType GetDir(void) const { return dir_; }
  ...
  */
  void Inverse(void) {
    switch (dir_) {
      case TenIndexDirType::IN:dir_ = TenIndexDirType::OUT;
        break;
  ...
  void Show(const size_t indent_level = 0) const override {
    ...
    std::string dir_str;
    switch (dir_) {
      case TenIndexDirType::IN:dir_str = "IN";
        break;
      case TenIndexDirType::NDIR:dir_str = "NDIR";
        break;
      case TenIndexDirType::OUT:dir_str = "OUT";
        break;
...
QNT CalcDiv(const IndexVec<QNT> &indexes, const CoorsT &blk_coors) {
  ...
  if (index0.GetDir() == TenIndexDirType::IN) {
    div = -div;
  }
  for (size_t i = 1; i < ndim; ++i) {
    auto index = indexes[i];
    if (index.GetDir() == TenIndexDirType::IN) {
      div += -index.GetQNSct(blk_coors[i]).GetQn();
    } else if (index.GetDir() == TenIndexDirType::OUT) {
      div += index.GetQNSct(blk_coors[i]).GetQn();
    }
  }
  return div;
...
#endif /* ifndef QLTEN_QLTENSOR_INDEX_H */
